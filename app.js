...
function render() {
    ...
    const ingScroll = ingList ? ingList.scrollTop : 0;
    const recipeScroll = recipeList ? recipeList.scrollTop : 0;
    ...
    if (state.route === 'pantry') {
        const newIngList = document.getElementById('ingredient-list');
        const newRecipeList = document.getElementById('recipe-list');
        if (newIngList && ingScroll) newIngList.scrollTop = ingScroll;
        if (newRecipeList && recipeScroll) newRecipeList.scrollTop = recipeScroll;
    }
}
...
function updateNavState() {
    ...
    if (pageContent) {
        pageContent.style.padding = (state.route === 'home' || state.route === 'login') ? '0' : '80px 0';
    }
}
    ...
    return `
        <div class="responsive-grid" style="gap: 40px; align-items: start;">
            <div class="glass glass-3d" style="padding: 40px;">
                ...
                <div id="ingredient-list" style="max-height: 600px; overflow-y: auto; padding-right: 12px;">
                    ${ingHTML}
                </div>
            </div>
            ...
            <div class="glass glass-3d" style="padding: 40px; background: rgba(255,255,255,0.85);">
                ...
                <div id="recipe-list" class="responsive-grid" style="gap: 24px; max-height: 600px; overflow-y: auto; padding-right: 12px;">
                    ${matchedRecipesHTML}
                </div>
            </div>
        </div>
